...
| Produced by | What |
|---|---|
| `tools/report.mjs` | totals, pass/fail/flaky/skipped, AC coverage table, failure list |
...
```bash
node tools/report.mjs results.json --ac AC1,AC2,AC3,AC4,AC5 --require-ac
```
...
- **Do not claim coverage the tool did not compute.** If `report.mjs` says an AC is NOT
  COVERED, the report says NOT COVERED — regardless of how thorough the work felt.
- **Do not recommend release when the gate failed.** A non-zero exit from `report.mjs` is
