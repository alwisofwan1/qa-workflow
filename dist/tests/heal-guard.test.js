...
test('swapping a locator is permitted — that is what healing is for', () => {
    clean(wrap(`  await expect(page.locator('.summary_value')).toContainText('SauceCard #31337')`), wrap(`  await expect(page.getByTestId('payment-info-value')).toContainText('SauceCard #31337')`), 'locator swap');
});
test('replacing a wait is permitted', () => {
    clean(wrap(`  await page.waitForSelector('#x')\n  await expect(page.getByRole('alert')).toBeVisible()`), wrap(`  await expect(page.getByRole('alert')).toBeVisible()`), 'wait removal keeps every assertion');
});
test('adding an assertion is permitted', () => {
    clean(wrap(`  await expect(page.getByRole('alert')).toBeVisible()`), wrap(`  await expect(page.getByRole('alert')).toBeVisible()\n  await expect(page.getByRole('alert')).toContainText('ok')`), 'added assertion');
});
test('weakening a matcher value is rejected — the real-world "Pony Express" -> "pony" heal', () => {
    flags(wrap(`  await expect(page.getByTestId('complete-text')).toContainText('Pony Express')`), wrap(`  await expect(page.getByTestId('complete-text')).toContainText('pony')`), /assertion changed or removed/, 'matcher value');
});
test('loosening a strict matcher is rejected', () => {
    flags(wrap(`  await expect(page.getByRole('cell')).toHaveText('Total: $97.17')`), wrap(`  await expect(page.getByRole('cell')).toContainText('Total')`), /assertion changed or removed/, 'toHaveText -> toContainText');
});
test('dropping a .not negation is rejected', () => {
    flags(wrap(`  await expect(page.getByTestId('badge')).not.toBeVisible()`), wrap(`  await expect(page.getByTestId('badge')).toBeVisible()`), /assertion changed or removed/, 'negation dropped');
});
test('changing a value asserted INSIDE expect() is rejected', () => {
    ...
    flags(wrap(`  expect(after.endsWith('\\n\\n' + before)).toBe(true)`), wrap(`  expect(after.endsWith('\\n' + before)).toBe(true)`), /inside expect\(\)/, 'blank-line rule gutted');
});
test('changing a number asserted inside expect() is rejected', () => {
    flags(wrap(`  expect(rows.length).toBeGreaterThan(3)`), wrap(`  expect(rows.length).toBeGreaterThan(0)`), /assertion changed or removed/, 'threshold weakened');
});
test('a literal inside a locator expression in expect() stays healable', () => {
    clean(wrap(`  await expect(page.getByRole('button', { name: 'Save' })).toBeEnabled()`), wrap(`  await expect(page.getByRole('button', { name: '保存' })).toBeEnabled()`), 'locator name change inside expect()');
});
test('deleting an assertion is rejected', () => {
    flags(wrap(`  await expect(a).toBeVisible()\n  await expect(b).toHaveText('x')`), wrap(`  await expect(a).toBeVisible()`), /assertion count dropped|assertion changed or removed/, 'assertion deleted');
});
test('commenting an assertion out is rejected', () => {
    flags(wrap(`  await expect(page.getByRole('alert')).toHaveText('saved')`), wrap(`  // await expect(page.getByRole('alert')).toHaveText('saved')`), /assertion count dropped|assertion changed/, 'assertion commented out');
});
...
test('expect.soft is counted like expect', () => {
    flags(wrap(`  expect.soft(total).toBe(97.17)\n  await expect(page.getByRole('alert')).toBeVisible()`), wrap(`  await expect(page.getByRole('alert')).toBeVisible()`), /assertion count dropped|assertion changed or removed/, 'soft assertion removed');
});
...
test('raising a matcher timeout is permitted — the asserted value is unchanged', () => {
    ...
    clean(wrap(`  await expect(page.getByRole('row')).toHaveCount(1, { timeout: 30_000 })`), wrap(`  await expect(page.getByRole('row')).toHaveCount(1, { timeout: 60_000 })`), 'timeout bump');
});
test('changing the asserted value is still rejected when options are present', () => {
    flags(wrap(`  await expect(page.getByRole('row')).toHaveCount(1, { timeout: 30_000 })`), wrap(`  await expect(page.getByRole('row')).toHaveCount(0, { timeout: 30_000 })`), /assertion changed or removed/, 'count changed with options');
});
test('an options-only matcher argument is ignored', () => {
    clean(wrap(`  await expect(page.getByRole('alert')).toBeVisible({ timeout: 5_000 })`), wrap(`  await expect(page.getByRole('alert')).toBeVisible({ timeout: 15_000 })`), 'options-only');
});
test('a constant imported from a fixture module is protected', () => {
    ...
    assert.ok(problems.some((p) => /named constant/.test(p)), `expected a violation, got ${JSON.stringify(problems)}`);
});
