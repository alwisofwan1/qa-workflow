...
let report: PwReport
try {
  report = JSON.parse(readFileSync(resultsPath, 'utf8')) as PwReport
} catch (err) {
...
if (base) {
  ...
  for (const path of specs) {
    ...
    if (problems.length) tamper.push({ path, problems })
  }
}
...
if (blocked) {
  ...
  lines.push(
    `All ${summary.tally.passed} test(s) passed and every declared acceptance criterion has a passing test.`
  )
}
if (tamper.length) {
